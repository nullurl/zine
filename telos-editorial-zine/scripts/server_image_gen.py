#!/usr/bin/env python3
"""Direct OpenAI-compatible image generation fallback for telos-editorial-zine.
...
def write_image(
    ...
    if item.get("url"):
        image_request = urllib.request.Request(
            item["url"], headers={"User-Agent": "telos-editorial-zine/1.0"}
        )
        try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", default="output/imagegen/telos-editorial-zine-output.png"
    )
