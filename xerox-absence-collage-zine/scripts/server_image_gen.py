#!/usr/bin/env python3
"""Generate a Xerox Absence Collage Zine image through an OpenAI-compatible API."""
...
def request_json(url: str, key: str, payload: dict, timeout: int) -> dict:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "xerox-absence-collage-zine/1.0",
        },
        method="POST",
    )
    try:
...
def write_item(item: dict, path: Path, timeout: int, force: bool) -> None:
    ...
    if item.get("b64_json"):
        ...
        request = urllib.request.Request(
            item["url"], headers={"User-Agent": "xerox-absence-collage-zine/1.0"}
        )
        try:
            with urllib.request.urlopen(request, timeout=timeout) as response:
