...
def write_image(
    ...
    if item.get("url"):
        image_request = urllib.request.Request(
            item["url"], headers={"User-Agent": "sumi-gesture-tattoo-zine/1.0"}
        )
        try:
...
def image_content(path_string: str) -> dict:
    path = Path(path_string)
    if not path.exists() or not path.is_file():
        die(f"Input image not found: {path}")
    mime_type = mimetypes.guess_type(path.name)[0] or "image/png"
    if mime_type not in {"image/png", "image/jpeg", "image/webp"}:
        die(f"Unsupported input image type: {path}")
...
def build_payload(args: argparse.Namespace, config: dict, prompt: str) -> tuple[str, dict]:
    ...
    if args.wire_api == "responses":
        ...
        content.extend(image_content(path) for path in args.input_image)
        payload = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", default="output/imagegen/sumi-gesture-tattoo-zine-output.png"
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Reference/edit image for Responses image generation; repeat as needed.",
    )
...
def main() -> None:
    ...
    if args.input_image:
        if args.wire_api == "images" and os.getenv("OPENAI_IMAGE_WIRE_API"):
            die("Input images require --wire-api responses.")
        args.wire_api = "responses"
